...
const SuggestedFriends = () => {
  ...
  const { data, isLoading } = useSuggestions(5, deferredSearch);
  ...
  const displayedSuggestions = allSuggestions.slice(0, 5);
  return (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm animate-in slide-in-from-top-2 duration-200">
          <div className="relative mb-3">
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-100 bg-slate-50 py-2 pl-9 pr-3 text-xs outline-none focus:border-[#1877f2] focus:bg-white"
            />
          </div>
          ...
          <div className="space-y-2">
            {isLoading ? (
              <Loader2
                className="mx-auto animate-spin text-[#1877f2] my-4"
                size={20}
              />
